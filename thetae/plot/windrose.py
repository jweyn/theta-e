...
# -*- coding: utf-8 -*-
"""
Module available on conda-forge as `windrose`:
https://github.com/python-windrose/windrose

Some modifications are my own, but I do not claim any rights.
"""
...
class WindroseAxes(PolarAxes):
    ...
    def legend(self, loc="lower left", decimal_places=1, **kwargs):
        ...
        def get_labels(decimal_places=1):
            _dp = str(decimal_places)

            labeln = np.copy(self._info['bins'])
            labels = ["%.{:s}f $-$ %0.{:s}f".format(_dp, _dp) % (labeln[i], labeln[i + 1])
                      for i in range(len(labeln) - 1)]
            if labeln[0] == 0:
                labels[0] = "< %0.{:s}f".format(_dp) % labeln[1]
            labels[-1] = "> %0.{:s}f".format(_dp) % labeln[-2]
            return labels
